...
test.describe(
  "SKIP_AUTH Mode Tests",
  { tag: ["@auth", "@skip-auth", "@e2e"] },
  () => {
    ...
    test("should bypass authentication and allow direct access", async ({
      page,
    }) => {
      ...
      try {
        ...
        await expect(page.locator("text=Loading graph...")).not.toBeVisible({
          timeout: 15000,
        });
      } catch {
      ...
      try {
        ...
        const graphContainer = page
          .locator('.graph-page, .center, [data-testid*="graph"]')
          .first();
    ...
    test("should work with API requests as test_user", async ({
      request,
      page,
    }) => {
    ...
    test("should not show login forms when SKIP_AUTH is enabled", async ({
      page,
    }) => {
    ...
    test("should allow access to profile page", async ({ page }) => {
      ...
      if (!isVisible) {
        ...
        const isLoadingVisible = await loadingState
          .isVisible()
          .catch(() => false);
        if (isLoadingVisible) {
    ...
    test("should handle concurrent requests as test_user", async ({
      request,
    }) => {
    ...
    test("should maintain SKIP_AUTH state across navigation", async ({
      page,
    }) => {
    ...
    test("should handle API errors gracefully in SKIP_AUTH mode", async ({
      request,
      page,
    }) => {
      // Try to access non-existent note
      const response = await request.get(
        `${getBackendUrl()}/api/v1/notes/non-existent-id`,
      );
