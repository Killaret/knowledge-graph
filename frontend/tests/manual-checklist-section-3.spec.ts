...
async function gotoGraph(page: Page) {
  ...
  await page.goto('/graph?nocache=1&full=1', { timeout: 60000, waitUntil: 'domcontentloaded' });
...
async function waitForGraphCanvas(page: Page): Promise<Locator> {
  ...
  await expect(page.locator('[data-testid="graph-stats"]')).toBeVisible({ timeout: 20000 });
...
async function getNodePositions(page: Page, filterNonTechnical = true): Promise<{ nodes: SimNode[]; transform: { x: number; y: number; k: number }; box: { x: number; y: number } }> {
  ...
  return { nodes: filterNonTechnical ? nodes.filter(n => n.type !== 'technical') : nodes, transform, box };
}

function toScreen(node: SimNode, transform: { x: number; y: number; k: number }, box: { x: number; y: number }) {
  return {
...
test.describe('Section 3 - Canvas Features', { tag: ['@manual', '@canvas'] }, () => {
  ...
  test('hotkeys open and close help, ghost form, and search', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="help-modal"]')).toBeVisible({ timeout: 5000 });
    await page.keyboard.press('Escape');
    await expect(page.locator('[data-testid="help-modal"]')).toBeHidden({ timeout: 5000 });

    // Ghost note form: N then Esc
    await page.keyboard.press('n');
    await expect(page.locator('[data-testid="ghost-note-form"]')).toBeVisible({ timeout: 5000 });
    await page.keyboard.press('Escape');
    await expect(page.locator('[data-testid="ghost-note-form"]')).toBeHidden({ timeout: 5000 });

    // Search box: F then Esc
    await page.keyboard.press('f');
    await expect(page.locator('[data-testid="search-box"]')).toBeVisible({ timeout: 5000 });
    await page.keyboard.press('Escape');
    await expect(page.locator('[data-testid="search-box"]')).toBeHidden({ timeout: 5000 });
  });
  ...
  test('ghost node creation adds a node to the graph', async ({ page }) => {
    ...
    const [response] = await Promise.all([
      page.waitForResponse((res) => res.url().includes('/api/v1/notes') && res.request().method() === 'POST'),
  ...
  test('clicking a node opens the side panel', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="note-side-panel"]')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('Delete key removes a selected node', async ({ page }) => {
    ...
    const [response] = await Promise.all([
      page.waitForResponse((res) => res.url().includes('/api/v1/notes/') && res.request().method() === 'DELETE'),
  ...
  test('drag-and-drop between nodes opens link form and creates a link', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="link-form"]')).toBeVisible({ timeout: 10000 });

    const [response] = await Promise.all([
      page.waitForResponse((res) => res.url().includes('/api/v1/links') && res.request().method() === 'POST'),
