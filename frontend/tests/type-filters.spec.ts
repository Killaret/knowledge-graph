...
test.describe(
  "Type Filters - Home Page Filtering",
  { tag: ["@smoke", "@filters"] },
  () => {
    ...
    test("should show all notes when selecting All filter", async ({
      page,
      request,
    }) => {
      const timestamp = Date.now();
    ...
    test("should filter graph nodes when type filter is applied", async ({
      page,
      request,
    }) => {
      const timestamp = Date.now();
      ...
      const planetNote = notesData.notes?.find((n: any) =>
        n.title?.includes(`Graph Planet ${timestamp}`),
      );
      const cometNote = notesData.notes?.find((n: any) =>
        n.title?.includes(`Graph Comet ${timestamp}`),
      );
      ...
      const graphContainer = page
        .locator(".fullscreen-graph, .graph-canvas, canvas")
        .first();
...
test.describe(
  "Type Filters - metadata.type fallback",
  { tag: ["@metadata"] },
  () => {
    test("should filter notes with type in metadata field", async ({
      page,
      request,
    }) => {
      const timestamp = Date.now();
    ...
    test("should fallback to star type when no type specified", async ({
      page,
      request,
    }) => {
      const timestamp = Date.now();
      ...
      const _notesResponse = await request.get(
