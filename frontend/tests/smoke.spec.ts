...
test.describe(
  ...
  () => {
    ...
    test("public access - main page loads with canvas and login button", async ({
      page,
    }) => {
      ...
      const graphContainer = page
        .locator(
          '[data-testid="graph-2d-container"], .fullscreen-graph, canvas',
        )
        .first();
    ...
    test("public access - profile redirects to login when not authenticated", async ({
      page,
    }) => {
    ...
    test("profile - page loads and displays user information", async ({
      page,
    }) => {
      // Setup SKIP_AUTH for testing
      await setupSkipAuth(page);
    ...
    test("graph - canvas loads with at least one node", async ({ page }) => {
      ...
      const graphCanvas = page
        .locator('[data-testid="graph-canvas"], canvas')
        .first();
    ...
    test("note creation - ghost node form appears and creates node", async ({
      page,
    }) => {
      // Setup SKIP_AUTH for testing
      await setupSkipAuth(page);
      ...
      const ghostForm = page
        .locator('[data-testid="ghost-node-form"], .ghost-node-form, .modal')
        .first();
      ...
      if (isFormVisible) {
        ...
        const submitButton = page
          .locator('button[type="submit"], text=Create, text=Создать')
          .first();
    ...
    test("logout - redirects to login and canvas becomes public", async ({
      page,
    }) => {
      // Setup SKIP_AUTH for testing
      await setupSkipAuth(page);
    ...
    test("note detail page - loads and displays note content", async ({
      page,
      request,
    }) => {
      // Setup SKIP_AUTH for testing
    ...
    test("search page - loads and displays search interface", async ({
      page,
    }) => {
      // Setup SKIP_AUTH for testing
      await setupSkipAuth(page);
      ...
      const searchInput = page
        .locator('input[type="search"], [data-testid="search-input"]')
        .first();
