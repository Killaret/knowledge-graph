...
async function waitForRouteContent(page: Page, route: string) {
  if (route === '/graph') {
    ...
    await expect(page.locator('[data-testid="profile-content"]')).toBeVisible({ timeout: 20000 });
  } else {
...
test.describe('Section 5 - General UX', { tag: ['@manual', '@ux'] }, () => {
  ...
  test('logout and re-login preserves graph data', async ({ page, request }) => {
    await page.goto('/graph?nocache=1', { timeout: 60000, waitUntil: 'domcontentloaded' });
  ...
  test('language switch in profile updates UI text', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Редактировать профиль' })).toBeVisible({ timeout: 10000 });
  });
