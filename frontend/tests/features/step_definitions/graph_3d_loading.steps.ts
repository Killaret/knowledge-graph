...
Then(
  "the fog density should be at least {float}",
  async function (this: ITestWorld, minDensity: number) {
...
Then(
  "the fog density should still be greater than {float}",
  async function (this: ITestWorld, threshold: number) {
    const density = await this.page.evaluate(() => {
      return (window as any).scene?.fog?.density ?? 0;
    });
...
Then(
  "the fog density should still be at least {float}",
  async function (this: ITestWorld, threshold: number) {
    const density = await this.page.evaluate(() => {
      return (window as any).scene?.fog?.density ?? 0;
    });
...
Then(
  "the fog density should become less than {float} within {int} second",
  async function (this: ITestWorld, maxDensity: number, seconds: number) {
...
When(
  "the simulation progresses to at least {int}% nodes positioned",
  async function (this: ITestWorld, percent: number) {
    try {
      await this.page.waitForFunction(
      ...
      const state = await this.page.evaluate(() => {
        ...
        return {
          total: nodes.length,
          positioned,
          progress: (positioned / nodes.length) * 100,
        };
      });
...
Given("the graph is still loading", async function (this: ITestWorld) {
  ...
  await this.page.waitForSelector(".loading-overlay, canvas", {
    timeout: 10000,
  });
