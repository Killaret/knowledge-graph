...
test.describe(
  "Auth Functional Tests (SKIP_AUTH Mode)",
  { tag: ["@auth", "@e2e"] },
  () => {
    test("should access application without authentication in SKIP_AUTH mode", async ({
      page,
    }) => {
    ...
    test("should show user interface as test_user", async ({ page }) => {
      ...
      const userMenu = page
        .locator('[data-testid="user-menu"], .user-menu, text=test_user')
        .first();
      const logoutBtn = page.locator("text=Logout").first();
      const userProfile = page
        .locator('[data-testid="user-profile"], .user-profile')
        .first();
      ...
      if (!hasUserUI) {
        const graphContainer = page
          .locator(".fullscreen-graph, canvas")
          .first();
        const graphAccessible = await graphContainer
          .isVisible()
          .catch(() => false);
    ...
    test("should protect auth routes when SKIP_AUTH is disabled", async ({
      page,
    }) => {
      ...
      await expect(page.locator("main, h1, .auth-card").first()).toBeVisible({
        timeout: 5000,
      });
    });
