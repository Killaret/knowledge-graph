...
- Name: home-page.spec.ts >> Home Page - Graph First >> should load notes and display them on graph
- Location: tests\home-page.spec.ts:28:3
...
Locator: locator('.graph-container canvas, .graph-canvas').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.graph-container canvas, .graph-canvas').first()
