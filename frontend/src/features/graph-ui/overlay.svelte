<script lang="ts">
  ...
  const {
    ...
    links: Array<{ source: string; target: string; link_type?: string; weight?: number; source_type?: string }>;
    loading?: boolean;
    hoveredNodeId?: string | null;
    hoveredLink?: { source: string; target: string; link_type: string; weight: number; source_type: string } | null;
    tooltipPosition?: { x: number; y: number };
    duplicateWarning?: { message: string; x: number; y: number; linkId: string } | null;
    focusMode?: boolean;
...
{#if hotkeysState.showSearchBox}
  ...
  >
    ...
    {/if}
    <button onclick={onCloseSearch} style="background: none; border: none; color: rgba(255,255,255,0.6); cursor: pointer; font-size: 16px;">×</button>
  </div>
{/if}
...
>
  {#if loading}
    <span style="display: inline-block; width: 12px; height: 12px; border: 2px solid rgba(255,255,255,0.3); border-top-color: white; border-radius: 50%; animation: spin 1s linear infinite;"></span>
  {/if}
