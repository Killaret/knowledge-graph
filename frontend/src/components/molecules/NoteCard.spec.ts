...
describe('NoteCard', () => {
	...
	it('renders with minimal note data', () => {
		const minimalNote = createNote({ title: 'Minimal', content: '', type: undefined });
	...
	it('renders selection checkbox and calls onSelect when toggled', async () => {
		...
		render(NoteCard, { props: { note: createNote(), selectMode: true, onSelect } });
		...
		expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ title: 'Test Note Title' }), true);
	});

	it('renders selected state when selected prop is true', () => {
		const { container } = render(NoteCard, { props: { note: createNote(), selected: true } });
