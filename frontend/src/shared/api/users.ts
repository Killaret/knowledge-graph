...
import type { User, UpdateUserRequest, APIKey } from "$shared/types";
...
export async function getSettings(): Promise<{
  ...
  const response = await api.get("v1/users/me/settings").json<{
    settings: Array<{ key: string; value: unknown; updated_at: string }>;
  }>();
  return response;
}
...
export async function getGalacticMode(): Promise<{ galactic_mode: boolean }> {
  const response = await api
    .get("v1/users/me/settings/galactic_mode")
    .json<{ galactic_mode: boolean }>();
  return response;
}
 ...
 */
export async function toggleGalacticMode(): Promise<{
  message: string;
  galactic_mode: boolean;
}> {
  const response = await api
    .post("v1/users/me/settings/galactic_mode/toggle")
    .json<{ message: string; galactic_mode: boolean }>();
  return response;
}
...
export async function getStreak(): Promise<{
  ...
  const response = await api
    ...
    .json<{ streak: number; next_reward: boolean }>();
  return response;
}
