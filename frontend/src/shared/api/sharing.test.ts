...
describe("sharing API", () => {
  ...
  it("shareNote creates a share", async () => {
    server.use(
      http.post(`${baseUrl}/v1/notes/n1/share`, () =>
  ...
  it("getNoteShares returns shares and links", async () => {
    ...
    server.use(
      http.get(`${baseUrl}/v1/notes/n1/shares`, () =>
  ...
  it("revokeShare removes a share", async () => {
    server.use(
      http.delete(`${baseUrl}/v1/notes/n1/shares/s1`, () =>
  ...
  it("revokeShareLink removes a share link", async () => {
    server.use(
      http.delete(`${baseUrl}/v1/share-links/l1`, () =>
        HttpResponse.json({}, { status: 204 }),
      ),
    );
  ...
  it("accessSharedNote returns shared note", async () => {
    ...
    server.use(
      http.get(`${baseUrl}/v1/share/abc`, () => HttpResponse.json(response)),
    );
