...
import { formatMessage, getCurrentLocale, setLocale, type Locale } from './i18n';
...
describe('i18n', () => {
  ...
  describe('formatMessage', () => {
    it('should return English message by default', () => {
      const result = formatMessage('note.created', 'en', { title: 'Test Note' });
      expect(result).toBe('Note "Test Note" created successfully.');
    });

    it('should return Russian message when locale is ru', () => {
      const result = formatMessage('note.created', 'ru', { title: 'Тестовая заметка' });
      expect(result).toBe('Заметка "Тестовая заметка" успешно создана.');
    });

    it('should replace single parameter', () => {
      const result = formatMessage('validation.error', 'en', { field: 'title' });
      expect(result).toBe('Invalid value in field "title".');
    });

    it('should replace multiple parameters', () => {
      const result = formatMessage('link.created', 'en', { source: 'A', target: 'B' });
