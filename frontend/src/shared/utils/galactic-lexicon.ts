  ...
  en: {
    standard: {
      success: {
        ...
        linkCreated: (source: string, target: string) => `Link from "${source}" to "${target}" created.`,
    ...
    galactic: {
      success: {
        ...
        linkCreated: (source: string, target: string) => `A gravity beam stretches from "${source}" to "${target}". 🌌`,
        ...
        accountDeleted: () => 'The galaxy has been consumed by a black hole. Everything created remains among the stars. 🌌',
        passwordChanged: () => 'Access code for the starship updated. 🔐',
      },
      error: {
        validation: (field: string) => `Sensors detected an anomaly in the field "${field}". Signal distorted. 📡`,
        ...
        serverError: () => 'Cosmic anomaly! Servers consumed by a supernova flare. Please try later. 🌋',
        networkError: () => 'Connection to the cosmic network lost. Check the hyperchannel. 📡',
        invalidCredentials: () => 'Invalid access code for the starship. The captain does not recognize you. 👨‍🚀',
      ...
      info: {
        emptyGraph: () => 'Your starry sky is empty. Create your first star to begin building the universe! 🌌',
        ...
        firstNoteHint: () => 'Be a pioneer! Create your first star in the universe of knowledge! ⭐',
        achievementProgress: (current: number, target: number) => `Stellar map filling: ${current} of ${target} objects discovered 🗺️`,
        streakActive: (days: number) => `Continuous journey: ${days} days without losing contact with base! 🚀`,
      ...
      warning: {
        ...
        deleteConfirm: (item: string) => `Are you sure you want to send "${item}" into a black hole? This is irreversible. 🕳️`,
...
export const GalacticLexicon = {
  success: {
    ...
    linkCreated: (source: string, target: string, useGalactic: boolean = false, locale: Locale = 'en') =>
  ...
  info: {
    ...
    achievementProgress: (current: number, target: number, useGalactic: boolean = false, locale: Locale = 'en') =>
