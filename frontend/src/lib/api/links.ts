...
export async function getLinks(): Promise<Link[]> {
  const response = await api.get('links').json<{ data: Link[] }>();
  return response.data;
}

// Получить связь по ID
export async function getLink(id: string): Promise<Link> {
  const response = await api.get(`links/${id}`).json<{ data: Link }>();
  return response.data;
}

// Создать новую связь
export async function createLink(data: CreateLinkData): Promise<Link> {
  const response = await api.post('links', { json: data }).json<{ data: Link }>();
  return response.data;
}
...
export async function getNoteLinks(noteId: string): Promise<Link[]> {
  const response = await api.get(`notes/${noteId}/links`).json<{ data: Link[] }>();
  return response.data;
}
