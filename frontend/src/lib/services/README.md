...
2. **После входа**: Предзагруженные данные немедленно отображаются, затем обновляются персональными данными
...
```typescript
import { getGraphWithPreload, getAchievementsWithPreload } from '$lib/hooks/usePreloadedData';
...
```typescript
import { useInstantData } from '$lib/hooks/usePreloadedData';
...
```typescript
import { loadAppData } from '$lib/hooks/usePreloadedData';
...
```typescript
import { PreloadService } from '$lib/services/PreloadService';
