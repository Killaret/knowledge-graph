...
describe('GraphCanvas - Link Rendering Detailed', () => {
  ...
  let ctxCalls: { method: string; args: any[]; fillStyle?: string; strokeStyle?: string; lineWidth?: number }[];
  ...
  beforeEach(() => {
    ...
    ctx = {
      ...
      stroke: vi.fn(() => ctxCalls.push({ method: 'stroke', args: [], strokeStyle: lastStrokeStyle, lineWidth: lastLineWidth })),
      ...
      set lineWidth(value: number) { lastLineWidth = value; },
  ...
  describe('Link Type Styles', () => {
    ...
    it('dependency link uses dashed orange line (#ff6600)', () => {
      ...
      const strokeCall = ctxCalls.find(c => c.method === 'stroke');
      expect(strokeCall).toBeDefined();
      expect(strokeCall?.strokeStyle).toContain('255, 102'); // rgba with orange component
    ...
    it('related link with strong weight uses solid gray line', () => {
      const sourceNode: SimulationNode = { id: '1', x: 100, y: 100, title: 'Source', type: 'star' };
      const targetNode: SimulationNode = { id: '2', x: 300, y: 200, title: 'Target', type: 'planet' };
      const link: SimulationLink = { source: '1', target: '2', weight: 0.8, link_type: 'related' };

      renderer.drawLink(ctx, link, sourceNode, targetNode);

      const strokeCall = ctxCalls.find(c => c.method === 'stroke');
      expect(strokeCall).toBeDefined();
      expect(strokeCall?.strokeStyle).toContain('153, 153'); // gray
    });

    it('related link with weak weight uses dashed gray line', () => {
      const sourceNode: SimulationNode = { id: '1', x: 100, y: 100, title: 'Source', type: 'star' };
      const targetNode: SimulationNode = { id: '2', x: 300, y: 200, title: 'Target', type: 'planet' };
      const link: SimulationLink = { source: '1', target: '2', weight: 0.2, link_type: 'related' };
  ...
  describe('Link Line Width', () => {
    it('line width increases with weight', () => {
      ...
      // High weight link
      const strongLink: SimulationLink = { source: '1', target: '2', weight: 1.0, link_type: 'reference' };
      ...
      // Low weight link
      const weakLink: SimulationLink = { source: '1', target: '2', weight: 0.3, link_type: 'reference' };
    ...
    it('dependency links have thicker lines', () => {
      ...
      // Dependency link
      const depLink: SimulationLink = { source: '1', target: '2', weight: 0.8, link_type: 'dependency' };
      ...
      const refLink: SimulationLink = { source: '1', target: '2', weight: 0.8, link_type: 'reference' };
  ...
  describe('Link Colors Helper Functions', () => {
    ...
    it('getLineDash returns correct patterns for each type', () => {
      expect(renderer.getLineDash('reference')).toEqual([]); // solid
      expect(renderer.getLineDash('dependency')).toEqual([10, 3]); // dash-dot
      expect(renderer.getLineDash('custom')).toEqual([2, 6]); // dotted
      expect(renderer.getLineDash('related', 0.2)).toEqual([6, 4]); // dashed for weak
      expect(renderer.getLineDash('related', 0.8)).toEqual([]); // solid for strong
    });
  });
});
