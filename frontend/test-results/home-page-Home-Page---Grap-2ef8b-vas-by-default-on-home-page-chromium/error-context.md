...
- Location: tests\home-page.spec.ts:18:3
...
```ts
  ...
  19  |     // Verify graph container is visible (fullscreen graph)
  20  |     const graphContainer = page.locator('.fullscreen-graph, .graph-canvas, canvas').first();
...
> 25  |     expect(containerHeight).toBeTruthy();
  ...
  54  |       const graphCanvas = page.locator('.fullscreen-graph canvas, .graph-canvas').first();
