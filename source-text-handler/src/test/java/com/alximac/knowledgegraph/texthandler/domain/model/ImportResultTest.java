...
class ImportResultTest {
    ...
    void mustCreateCompleted() {
        ImportResult result = new ImportResult(
                VALID_CORR_ID, "evt-1", ImportResult.Status.COMPLETED,
    ...
    void mustCreatePartial() {
        ImportResult result = new ImportResult(
                VALID_CORR_ID,"evt-2", ImportResult.Status.PARTIAL,
    ...
    void mustRejectNullFields() {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(null,"evt-3", ImportResult.Status.COMPLETED, List.of(), List.of(), List.of(), NOW))
                .withMessageContaining("Correlation id must not be null or blank");

        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(VALID_CORR_ID,"evt-3", null, List.of(), List.of(), List.of(), NOW))
                .withMessageContaining("Status must not be null");

        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(VALID_CORR_ID,"evt-3", ImportResult.Status.COMPLETED, null, List.of(), List.of(), NOW))
                .withMessageContaining("NoteIds must not be null");

        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(VALID_CORR_ID,"evt-3", ImportResult.Status.COMPLETED, List.of(), null, List.of(), NOW))
                .withMessageContaining("Links must not be null");

        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(VALID_CORR_ID,"evt-3", ImportResult.Status.COMPLETED, List.of(), List.of(), null, NOW))
                .withMessageContaining("Errors must not be null");

        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(VALID_CORR_ID,"evt-3", ImportResult.Status.COMPLETED, List.of(), List.of(), List.of(), null))
    ...
    void mustRejectBlankCorrelationId() {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> new ImportResult(" ","evt-4", ImportResult.Status.COMPLETED, List.of(), List.of(), List.of(), NOW))
                .withMessageContaining("Correlation id must not be null or blank");
    }
    ...
    void mustMakeCollectionsUnmodifiable() {
        ImportResult result = new ImportResult(
                VALID_CORR_ID,"evt-5", ImportResult.Status.COMPLETED,
