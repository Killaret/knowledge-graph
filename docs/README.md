...
**Сверено 2026-09-08.** Все 60 документов верхнего уровня перечислены в этом указателе, все локальные ссылки разрешаются, все упомянутые команды `npm run` существуют. Проверено машинно — фаза `Documentation links` в [`check-all`](../COMMANDS.md) и в CI, скрипт [`check-docs-links.mjs`](../scripts/testing/check-docs-links.mjs).
...
| [AI_AGENT_SETUP.md](AI_AGENT_SETUP.md) | Из чего состоит обвязка агентов и как развернуть её на другой машине |
