...
- Документировать в `docs/archive/CRITICAL_FIXES.md` или `docs/operations/REGRESSION_TEST_PLAN.md`, что `+page.svelte` и `graph/+page.svelte` содержат дублирующий CRUD-код.
- Отслеживать FSD-нарушение `GraphCanvas.svelte` в `docs/archive/CRITICAL_FIXES.md`.
