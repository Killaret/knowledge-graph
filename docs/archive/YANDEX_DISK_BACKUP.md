...
> **⚠️ Устаревший путь.** Решение 31 ([`DECISIONS.md`](../DECISIONS.md), 2026-09-14):
> облачный бэкап Personal-стека идёт через **синхронизируемую папку**
> `%USERPROFILE%\Desktop\my items`, а не через API — OAuth-токен не нужен.
> REST-API код (`BACKUP_CLOUD_ENABLED=true`) сохранён и работает, но по
> умолчанию выключен везде. Актуальный документ — [`BACKUP.md`](../operations/BACKUP.md).
