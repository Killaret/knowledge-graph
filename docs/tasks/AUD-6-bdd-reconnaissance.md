...
**Принято.** Коммит `cc09058`. Задача была на измерение, и измерение выполнено.

Отчёт даёт ровно то, что требовалось: разделение прогонов вместо попытки объединить несовместимые наборы, цифры по обоим, классификация 88 падений по причинам с оценкой объёма в файлах, перечень механических конфликтов и способ их обхода, ответ про уникальное покрытие и явная рекомендация. Подмена `support/` временными копиями вместо правки боевых — верное решение: основное дерево не тронуто.

Существенное из отчёта, чего не было в постановке: активный набор тоже меньше, чем считалось. Из девяти сценариев в skip-auth режиме исполняются пять, остальные четыре — `@auth-real`.

Ключевая цифра для решения владельца: **8 проходит, 88 падает, 29 не запускается**, и главная причина — не селекторы, а устаревший backend URL в 76 сценариях. То есть набор не «сгнил», а отстал от переезда API; починка массовая, но механическая.

Рекомендацию исполнителя — подключать частично и отдельной задачей — поддерживаю. Уникальное покрытие (achievements, 3D-камера, типы небесных тел, типы связей, одиночный импорт/экспорт, семантический поиск) реально не покрыто ни Playwright, ни активным BDD, и удалять его было бы потерей.
