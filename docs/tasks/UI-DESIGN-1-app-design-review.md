...
**Владелец, 2026-09-26: сделать так, как он это видел.** Образец — то же описание в
`FRONTEND_ARCHITECTURE_EN.md`:

- узлы проявляются из тумана порциями, с задержкой между ними — в описании по 5 с паузой 200 мс;
- в начале туман плотный и редеет по мере загрузки;
- строка статистики показывает прогресс и число узлов;
- камера подстраивается под появившиеся узлы.

История (git, проверено 2026-09-26) — замысел не сломался, его потеряли:

- 3D: постепенная загрузка из тумана сделана 2026-04-15 (`d6b0658`, «implement Progressive Graph
  Rendering with fog of war effect») на странице `/graph/3d/[id]`: туман 0,08 на старте редел за
  2 с, узлы добавлялись в симуляцию постепенно (`lib/three/simulation/forceSimulation.ts`,
  `lib/components/Graph3D.svelte`). Удалена 2026-05-20 вместе со старым 3D-модулем (`86e66d9`,
  «Clean up junk and obsolete files») — без решения и без правки документации. Новый движок
  `features/graph-3d/` её не унаследовал.
- 2D: загрузки через туман не было никогда. Туман 2D (`89e4592`, 2026-08-07) зависит от частоты
  кадров.
- Оверлей «Loading notes…» на весь экран появился 2026-07-05 (`9c13431`).

Старую реализацию смотреть как образец поведения (`git show d6b0658`), а не переносить: движок
другой. Как такое искать по всей документации — [`DOC-AUDIT-2-docs-vs-code.md`](DOC-AUDIT-2-docs-vs-code.md).
