...
В корне `D:\` найдены 10 документов второго пакета от внешнего AI-ассистента (Gordon) — «Deployment Optimization Package» для Knowledge Graph (~4 700 строк). Файлы собраны в `docs/archive/gordon/` рядом с первым пакетом (см. GORDON-1). Их нужно прочитать, сверить с принятыми решениями и доской, и решить, что встроить в проект.

## Источники

- [`docs/archive/gordon/gordon_index.md`](../archive/gordon/gordon_index.md) — точка входа пакета, карта документов.
- [`docs/archive/gordon/gordon_complete_package.md`](../archive/gordon/gordon_complete_package.md) — свод пакета (89 КБ, 7 файлов).
- [`docs/archive/gordon/gordon_files_summary.md`](../archive/gordon/gordon_files_summary.md) — перечень файлов с назначением.
- [`docs/archive/gordon/gordon_visual_summary.md`](../archive/gordon/gordon_visual_summary.md) — визуальное резюме.
- [`docs/archive/gordon/gordon_deployment_detailed_guide.md`](../archive/gordon/gordon_deployment_detailed_guide.md) — подробный deployment-гайд (736 строк).
- [`docs/archive/gordon/gordon_infrastructure_research.md`](../archive/gordon/gordon_infrastructure_research.md) — инфраструктурное исследование и стратегия (810 строк).
- [`docs/archive/gordon/gordon_infrastructure_discussion_guide.md`](../archive/gordon/gordon_infrastructure_discussion_guide.md) — гайд для обсуждения решений.
- [`docs/archive/gordon/gordon_kubernetes_setup.md`](../archive/gordon/gordon_kubernetes_setup.md) — Kubernetes и мониторинг для production.
- [`docs/archive/gordon/gordon_production_checklist.md`](../archive/gordon/gordon_production_checklist.md) — чек-лист production и нагрузочное тестирование.
...
5. Итоговый вердикт по `docs/archive/gordon/` совместно с вопросом 4 из GORDON-1: оставить как архив внешних анализов или влить выводы и удалить.
...
- **Kubernetes-трек — отложено.** Остаёмся на Docker Compose (`docker-compose.deploy.yml` + `deploy.yml` на Docker Hub). K8s-манифесты и Prometheus-стек оставлены в `docs/archive/gordon/` как справочник на будущее.
