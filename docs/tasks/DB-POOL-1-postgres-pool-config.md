...
- `config/backend.json` — новая секция `database.pool`: `max_open_conns: 25`, `max_idle_conns: 5`, `conn_max_lifetime_seconds: 300`, `conn_max_idle_time_seconds: 60`. Дефолты = прежние захардкоженные значения, поведение не меняется.
...
- `go build ./...` — OK; `go test ./internal/infrastructure/db ./internal/config ./cmd/...` — PASS, включая новые unit-тесты `normalizePoolConfig` (zero/negative → дефолты, частичный override, кастомные значения) и `ConnectWithPool` на пустом DSN.
