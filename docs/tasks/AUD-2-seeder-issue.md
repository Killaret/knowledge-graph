...
Конструкция разбиралась прямо: режим обхода авторизации опирается на реальную строку
пользователя, потому что `notes.creator_id` — внешний ключ. Убрать строку значит либо
сломать режим, либо развязать внешний ключ; ни то, ни другое не окупается ради
тестового контура. Принято как есть.

Решение принималось на доске и до 2026-09-14 существовало только там — в этом файле
его не было, хотя указатель `docs/DECISIONS.md` ссылался сюда. Записано здесь, чтобы
ссылка вела к решению, а не только к описанию дефекта.
