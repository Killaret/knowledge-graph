...
В проекте реализована поддержка резервного копирования базы данных на Яндекс.Диск через WebDAV API.
...
- **WebDAV API**: прямая загрузка файлов на Яндекс.Диск через OAuth токен
...
## Сравнение с Cloudflare R2

| Характеристика | Яндекс.Диск | Cloudflare R2 |
|--------------|-------------|--------------|
| Бесплатное место | 10 ГБ | 10 ГБ/месяц |
| Регистрация | Простой аккаунт Яндекс | Требуется Cloudflare аккаунт |
| API | WebDAV (простой) | S3 API |
| Сложность настройки | Низкая | Средняя |
| Для кого | Личное использование | Проекты и бизнес |

## Дополнительные возможности

### Изменение папки для бэкапов

В скриптах бэкапа папка по умолчанию: `/knowledge-graph-backups`

Чтобы изменить, отредактируйте скрипт:
- Windows: строка 55 в `backup-personal.ps1`
- Linux/Mac: строка 53 в `backup-personal.sh`
