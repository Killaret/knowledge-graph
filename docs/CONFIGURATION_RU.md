...
> дефолт из Go. Пример: убрать `gamma_link_min_score` из JSON — получится
> `GAMMA_LINK_MIN_SCORE = 0.0`, порог отключён, а не «по умолчанию». Поэтому
> новые ключи обязаны попадать в `config/*.json`; фолбэк в Go — только
> страховка на случай запуска без файла.
