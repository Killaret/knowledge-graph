...
> **Дефолты из кода работают только когда JSON-конфиг не загружен вообще.**
> Если `knowledge-graph.config.json` существует, `getJSONFloatOrDefault`/
> `getJSONStringOrDefault` возвращают сырое поле — *отсутствующий ключ*
> десериализуется как нулевое значение (`0`, `""`, `false`) и молча затирает
> дефолт из Go. Поэтому новые ключи обязаны попадать в `config/*.json`;
> фолбэк в Go — только страховка на случай запуска без файла.
> Исключение: `gamma_link_min_score` защищён в коде — значение `<= 0`
> (в том числе от отсутствующего ключа) заменяется дефолтом `0.6` со строкой
> в логе, потому что порог 0 означал бы «связать каждую заметку с кем угодно»
> (LINKS-1, доработка).
