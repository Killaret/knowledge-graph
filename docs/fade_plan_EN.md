...
2. In `startSimulation` (`frontend/src/lib/components/GraphCanvas/simulation.ts`) on start:
...
3. In the renderer (`frontend/src/lib/components/GraphCanvas/renderer.ts`) consider opacity from `Map` when drawing (`ctx.globalAlpha` or `rgba(...)`) and trigger redraw on changes.
...
- `frontend/src/lib/components/Graph3D/fogManager.ts` (reference)
...
- ✅ Tests: added `frontend/src/lib/components/GraphCanvas.fade.spec.ts`, asserting initial zero opacity, progression, and final approach to `1`.
