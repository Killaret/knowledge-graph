...
|------------|-----|----------|--------------|----------|
| `RECOMMENDATION_ALPHA` | backend | Вес явных связей (BFS) в финальной оценке | `0.7` | 0.0 - 1.0 |
| `RECOMMENDATION_BETA` | backend | Вес семантического сходства (эмбеддинги) | `0.3` | 0.0 - 1.0 |
...
| Переменная | Где | Описание | По умолчанию | Диапазон/Варианты |
|------------|-----|----------|--------------|-------------------|
| `RECOMMENDATION_GAMMA` | backend | Дополнительный коэффициент для третьего компонента в формуле рекомендаций | `0.2` | 0.0 - 1.0 |
| `BFS_AGGREGATION` | backend | Метод агрегации весов при BFS-обходе | `max` | `max`, `sum`, `avg` |
| `BFS_NORMALIZE` | backend | Нормализация весов связей в BFS | `true` | `true`, `false` |
| `ASYNQ_CONCURRENCY` | worker | Уровень параллелизма обработки задач Asynq | `10` | 1 - 50 |
| `ASYNQ_QUEUE_DEFAULT` | worker | Приоритет дефолтной очереди Asynq | `1` | 1 - 10 |

### Детальное описание новых параметров

**RECOMMENDATION_GAMMA** — Третий коэффициент в формуле рекомендаций:
```
score = α × explicit_score + β × semantic_score + γ × additional_score
```
