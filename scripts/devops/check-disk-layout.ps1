# check-disk-layout.ps1 - DISK-1 guard: heavy tool caches must not live on C:.
...
function Test-PathOnC([string]$label, [string]$path, [string]$how) {
    ...
    if ($p -match '^[a-zA-Z]:') {
        if ($p.StartsWith('C:', [StringComparison]::OrdinalIgnoreCase)) {
            ...
            Write-Host ("  {0,-28} {1}  <-- ON C:" -f $label, $p)
        } else {
...
Write-Host "== Configured tool paths (must be off C:) =="
...
Test-PathOnC 'go GOCACHE'      $goEnv['GOCACHE']   'go env -w GOCACHE=D:\kg-build-cache\go-build'
Test-PathOnC 'go GOMODCACHE'   $goEnv['GOMODCACHE'] 'go env -w GOMODCACHE=D:\kg-build-cache\go-mod'
Test-PathOnC 'go GOTMPDIR'     $goEnv['GOTMPDIR']   'go env -w GOTMPDIR=D:\kg-build-cache\tmp'

$npmCache = (npm config get cache 2>$null | Select-Object -First 1)
Test-PathOnC 'npm cache' $npmCache 'npm config set cache D:\kg-build-cache\npm --location=user'

$pipCache = (pip cache dir 2>$null | Select-Object -First 1)
Test-PathOnC 'pip cache' $pipCache 'pip config set global.cache-dir D:\kg-build-cache\pip'
...
Test-PathOnC 'PLAYWRIGHT_BROWSERS_PATH' (Get-UserEnv 'PLAYWRIGHT_BROWSERS_PATH') 'setx PLAYWRIGHT_BROWSERS_PATH D:\kg-build-cache\ms-playwright'
Test-PathOnC 'PUPPETEER_CACHE_DIR'      (Get-UserEnv 'PUPPETEER_CACHE_DIR')      'setx PUPPETEER_CACHE_DIR D:\kg-build-cache\puppeteer'
Test-PathOnC 'TORCH_HOME'               (Get-UserEnv 'TORCH_HOME')               'setx TORCH_HOME D:\kg-build-cache\torch'
Test-PathOnC 'HF_HOME'                  (Get-UserEnv 'HF_HOME')                  'setx HF_HOME D:\kg-hf-cache'
...
if (Test-Path $envFile) {
    ...
    Test-PathOnC '.env HF_CACHE_DIR' $hfDir '.env: HF_CACHE_DIR=D:/kg-hf-cache'
} else {
    Write-Host "  .env HF_CACHE_DIR            (.env absent - cp .env.example .env)"
}

Write-Host ""
Write-Host "== Known heavy dirs still on C: (sizes) =="
...
Write-Host ""
if ($fail.Count -gt 0) {
    Write-Host "FAIL: $($fail.Count) configured path(s) still point at C::"
    $fail | ForEach-Object { Write-Host "  - $_" }
    exit 1
}
Write-Host "OK: all configured tool caches are off C:"
exit 0
